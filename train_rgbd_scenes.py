...
def main():
    ...
    if not args.debug:
        ...
        tracker = ComposedTracker([
            Tracker(subset="test_rgb_scenes", target="mse", snapshoter=snapshoter, eps=0.01),
            Tracker(subset="val_rgb_scenes", target="mse", snapshoter=snapshoter, eps=0.01),
        ])
    else:
    ...
    train_dataset = CompletionDataset(RGBD_Scenes(split="train"))
    ...
    val_datasets = {
        "val_rs": CompletionDataset(Realsense(split="val")),
        "test_nyu": CompletionDataset(RGBD_Scenes(split="test")),
    }
    val_loaders = {
